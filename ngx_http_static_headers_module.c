...
static char* ngx_http_static_headers(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static ngx_int_t ngx_http_static_headers_preconf(ngx_conf_t *cf);
static ngx_int_t ngx_http_static_headers_postconf(ngx_conf_t *cf);

static void* ngx_http_static_headers_create_loc_conf(ngx_conf_t *cf);

static char* ngx_http_static_headers_merge_loc_conf(ngx_conf_t *cf,
    void *parent, void *child);

typedef struct {
    ngx_flag_t  enable;
} ngx_http_static_headers_loc_conf_t;

static ngx_command_t  ngx_http_static_headers_commands[] = {
    { ngx_string("static_headers"),
      NGX_HTTP_LOC_CONF|NGX_CONF_FLAG,
      ngx_http_static_headers,
...
static ngx_http_module_t  ngx_http_static_headers_module_ctx = {
    ...
    ngx_http_static_headers_create_loc_conf,  /* create location configuration */
    ngx_http_static_headers_merge_loc_conf /* merge location configuration */
};


ngx_module_t  ngx_http_static_headers_module = {
    NGX_MODULE_V1,
    &ngx_http_static_headers_module_ctx, /* module context */
    ngx_http_static_headers_commands,   /* module directives */
...
static ngx_int_t
ngx_http_static_headers_handler(ngx_http_request_t *r)
{
    size_t root;
    ngx_str_t path;
    ...
    ngx_http_static_headers_loc_conf_t  *cglcf;
    cglcf = ngx_http_get_module_loc_conf(r, ngx_http_static_headers_module);
...
//   headers to new file (name = requested file)
	...
	//if (of.is_dir) {
	//    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Trying to headers dir: %s", path.data);
        //    return NGX_DECLINED;
	//}

	// Split requested URI into component file names & attach '.js' if needed
	u_char *files[10]; // 10 = max files to headers, TODO: make config var or something
	u_int ii = 0, jj = 0;
...
*/
	...
	// TODO: write path.data to headers log - log file should be a config var
	...
	r->headers_out.content_length_n = 100; // TODO: Size of headersted file
	...
	// TODO: read in headersted file & stuff into buffer
...
static char *
ngx_http_static_headers(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_core_loc_conf_t  *clcf;
    ngx_http_static_headers_loc_conf_t *cglcf = conf;


    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
    clcf->handler = ngx_http_static_headers_handler;
...
static void *
ngx_http_static_headers_create_loc_conf(ngx_conf_t *cf)
{
    ngx_http_static_headers_loc_conf_t  *conf;

    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_static_headers_loc_conf_t));
    if (conf == NULL) {
...
static char *
ngx_http_static_headers_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ngx_http_static_headers_loc_conf_t *prev = parent;
    ngx_http_static_headers_loc_conf_t *conf = child;
